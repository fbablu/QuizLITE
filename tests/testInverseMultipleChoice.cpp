...
protected:
  ...
  void TearDown() override { UserSession::resetInstance(); }
...
TEST_F(InverseMultipleChoiceTest, InitializationWithFewEntries) {
  ...
  std::set<std::string> expectedAnswers = {"Answer1", "Answer2", "Answer3",
                                           "Answer4"};
...
TEST_F(InverseMultipleChoiceTest, GenerateOptionsForSmallSet) {
  ...
  std::unordered_set<std::string> uniqueOptions{
      std::get<0>(options), std::get<1>(options), std::get<2>(options),
      std::get<3>(options)};
...
TEST_F(InverseMultipleChoiceTest, GenerateOptionsForLargeSet) {
  ...
  for (int i = 5; i <= 10; ++i) {
    userSession->addToStudySet("TestSet", "Question" + std::to_string(i),
